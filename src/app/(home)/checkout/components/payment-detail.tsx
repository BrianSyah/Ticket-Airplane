...
interface PaymenDetailProps {
  user: User | null;
}

export default function PaymentDetail({ user }: PaymenDetailProps) {
  const { data } = useCheckoutData();

  const selectedSeat = useMemo(() => {
    return SEAT_VALUES[(data?.seat as SeatValuesType) ?? "ECONOMY"];
  }, [data?.seat]);
  const { payTransaction, isLoading } = useTransaction({ user });
  return (
    <div className="flex flex-col gap-[30px] w-[400px]">
      <div className="flex flex-col gap-[18px]">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold">Free(Rp 0)</span>
        </div>
        <div className="flex justify-between">
          <span>Baggage</span>
          <span className="font-semibold">Free(Rp 0)</span>
